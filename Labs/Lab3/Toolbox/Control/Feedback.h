...
#define _FEEDBACK_H
  ...
  #define Init_FeedbackControlData(fbkc, tau) do{ \
    ...
    fbkc.corrective_time_recip = 1.0 / ((float) tau); \
    /*fbkc.t_last = 0.0;*/ \
  ...
  ****/
  void getControl_fbk_time(Vector3x1* Vprof, FeedbackControlData* fbkc, TPose* Pcomm){
    ...
    wrp_x = (Pcomm->X - rob_pos->X);
    wrp_y = (Pcomm->Y - rob_pos->Y);

    float ex, ey, eth; // Position of Target Point, P, Relative to Robot in Robot-Frame
    ex = cos(rob_pos->TH) * wrp_x + sin(rob_pos->TH) * wrp_y;
    ey = cos(rob_pos->TH) * wrp_y - sin(rob_pos->TH) * wrp_x;
    eth = adel(Pcomm->TH, rob_pos->TH);
